...
Citizen.CreateThread(function()
    while true do
        ...
        local pos = GetEntityCoords(GetPlayerPed(-1))
                if (GetDistanceBetweenCoords(pos, 441.85, -982.77, 30.94, true) < 2) then
                    if (GetDistanceBetweenCoords(pos, 441.85, -982.77, 30.94, true) < 2.3) then  
                        DrawText3D(441.85, -982.77, 30.94 + 0.1, "If No One Is Here, Page Us!")          
                        DrawText3D(441.85, -982.77, 30.94, "Do not spam button!")
                        DrawText3D(441.85, -982.77, 30.94 - 0.1, "Press ~g~E~w~ to Page an Officer!")
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("qb-policejob:server:SendPDAlert")
                            QBCore.Functions.Notify("An Officer should be here shortly. Wait Around!")
                        end
                    end
                elseif (GetDistanceBetweenCoords(pos, 441.85, -982.77, 30.94, true) < 2) then
                    DrawText3D(441.85, -982.77, 30.94, "Page an Officer")
                end
    end
end)

RegisterNetEvent('qb-policejob:client:SendPDAlert')
AddEventHandler('qb-policejob:client:SendPDAlert', function(msg)
    PlaySound(-1, "Menu_Accept", "Phone_SoundSet_Default", 0, 0, 1)
    TriggerEvent("chatMessage", "Front Desk", "success", msg)
end)
...
RegisterNetEvent('qb-police:client:spawnHelicopter', function(k)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            ...
            SetVehicleNumberPlateText(veh, "AIRU"..tostring(math.random(1000, 9999)))
            SetEntityHeading(veh, coords.w)
...
if Config.UseTarget then
    CreateThread(function()
        -- Toggle Duty
            exports['qb-target']:AddBoxZone("PoliceDuty_", vector3(441.81, -981.93, 30.87), 1, 1, {
                name = "PoliceDuty_",
                heading = 11,
                debugPoly = false,
                minZ = 29.7,
...
--[[CreateThread(function()
    ...
    RegisterNetEvent('police:client:ArchiveStashBox', function(data)
        local currentArchive = data.currentArchive
        local pos = GetEntityCoords(PlayerPedId())
        local takeLoc = Config.Locations["archive"][currentArchive]
    
        if not takeLoc then return end
       -- if #(pos - takeLoc) <= 1.0 then
            local box = exports['qb-input']:ShowInput({
                header = 'Archive Stash | '.. currentArchive,
                submitText = "open",
                inputs = {
                    {
                        type = 'number',
            ...
            if box then
                if not box.slot then return end
                TriggerServerEvent("inventory:server:OpenInventory", "stash", currentArchive.." | Box "..box.slot, {
                    maxweight = 4000000,
                    slots = 500,
                })
                TriggerEvent("inventory:client:SetCurrentStash", currentArchive.." | Box "..box.slot)
            end
       -- else
            --exports['qb-menu']:closeMenu()
        --end
...
CreateThread(function()
    ...
    while true do
        local sleep = 2000
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "police" or PlayerJob.name == "bcso" then
            local pos = GetEntityCoords(PlayerPedId())
            for k, v in pairs(Config.Locations["evidence"]) do
                if #(pos - v) < 2 then
                    sleep = 5
...
CreateThread(function()
    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "police" then
            local pos = GetEntityCoords(PlayerPedId())
            for k, v in pairs(Config.Locations["archive"]) do
                if #(pos - v) < 2 then
                    sleep = 5
                    if #(pos - v) < 1.0 then
                        if not headerDrawn then
                            ...
                            exports['qb-menu']:showHeader({
                                {
                                    header = 'Archive Stash | ' .. k,
                                    params = {
                                        event = 'police:client:ArchiveStashBox',
                                        args = {
                                            currentArchive = k
                                        }
                                    }
...
RegisterNetEvent("qb-policejob:client:target:OpenStash")
AddEventHandler("qb-policejob:client:target:OpenStash", function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "filingcabinetLSPD_001", {maxweight = 100000, slots = 100})
    TriggerEvent("inventory:client:SetCurrentStash", "filingcabinetLSPD_001")
end)
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['qb-target']:AddBoxZone("stash_"..k, vector3(v.x, v.y, v.z), 1, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-policejob:client:target:OpenStash",
                        icon = "fas fa-sign-in-alt",
                        label = "Open Filing Cabinet",
                        job = {
                            ["police"] = 8,
                        }
                    },
    ...
    for k, v in pairs(Config.Locations["helicopter"]) do
        exports['qb-target']:AddBoxZone("Helicopter_"..k, vector3(v.x, v.y, v.z), 10, 10, {
            name = "Helicopter_"..k,
            heading = 11,
            debugPoly = false,
            minZ = v.z - 2,
            ...
            options = {
                {
                    type = "client",
                    event = "qb-police:client:spawnHelicopter",
                    icon = "fas fa-sign-in-alt",
