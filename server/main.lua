...
RegisterServerEvent('qb-policejob:server:SendPDAlert')
AddEventHandler('qb-policejob:server:SendPDAlert', function()
	local src = source
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		local Player = QBCore.Functions.GetPlayer(v)
		if Player ~= nil then 
			if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
				TriggerClientEvent("qb-bestbuds:client:SendAlert", v, "An Officer is needed at MRPD")
			end
		end
	end
end)
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "bcso" or Player.PlayerData.job.name == "sapr" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
        if args[2] == "driver" or args[2] == "weapon" or args[2] == "weapon2" or args[2] == "cdl_license" or args[2] == "fishinggame_license" or args[2] == "piloting_license" or args[2] == "business_license" or args[2] == "boating_license" or args[2] == "motorcycle_license" then
...
QBCore.Commands.Add("lspdobject", Lang:t("commands.place_object"), {{name = "type",help = Lang:t("info.poobject_object")}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        ...
        elseif type == "iv" then
            TriggerClientEvent("police:client:spawnIV", src)
...
QBCore.Commands.Add("unjail", Lang:t("commands.unjail_player"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "bcso" and Player.PlayerData.job.onduty then
        ...
        TriggerClientEvent("pickle_prisons:unjailPlayer", playerId)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
end)
...
QBCore.Commands.Add("pdimpound", Lang:t("commands.impound"), {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "bennys" or Player.PlayerData.job.name == "mechanic" and Player.PlayerData.job.onduty then
...
QBCore.Commands.Add("paytow", Lang:t("commands.paytow"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "tow" or OtherPlayer.PlayerData.job.name == "mechanic" or OtherPlayer.PlayerData.job.name == "srpmechanic" or OtherPlayer.PlayerData.job.name == "bennys" or OtherPlayer.PlayerData.job.name == "hayes" then
                OtherPlayer.Functions.AddMoney("bank", 2500, "police-tow-paid")
...
QBCore.Commands.Add("paylawyer", Lang:t("commands.paylawyer"), {{name = "id",help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "judge" then
        ...
        if OtherPlayer.PlayerData.job.name == "lawyer" then
            OtherPlayer.Functions.AddMoney("bank", 4500, "police-lawyer-paid")
            TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("success.tow_paid"), 'success')
...
QBCore.Functions.CreateUseableItem("panicbutton", function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    TriggerClientEvent("police:client:PanicButton", src)
end)
...
QBCore.Functions.CreateUseableItem("slimjimtools", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent("lockpicks:UseSlimJim", source, true)
end)


RegisterNetEvent('qb-policejob:server:setVehLockState', function(vehNetId, state)
...
RegisterNetEvent('police:server:policeAlert', function(text)
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'police' or v.PlayerData.job.name == 'sheriff' and v.PlayerData.job.onduty then
...
RegisterNetEvent('police:server:mechanicAlert', function(text)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'mechanic' and v.PlayerData.job.onduty then
            local alertData = {title = Lang:t('info.new_call'), coords = {x = coords.x, y = coords.y, z = coords.z}, description = text}
            TriggerClientEvent("qb-phone:client:addPoliceAlert", v.PlayerData.source, alertData)
            TriggerClientEvent('police:client:policeAlert', v.PlayerData.source, coords, text)
        elseif v and v.PlayerData.job.name == 'srpmechanic' and v.PlayerData.job.onduty then
            local alertData = {title = Lang:t('info.new_call'), coords = {x = coords.x, y = coords.y, z = coords.z}, description = text}
            TriggerClientEvent("qb-phone:client:addPoliceAlert", v.PlayerData.source, alertData)
            TriggerClientEvent('police:client:policeAlert', v.PlayerData.source, coords, text)
        elseif v and v.PlayerData.job.name == 'bennys' and v.PlayerData.job.onduty then
            local alertData = {title = Lang:t('info.new_call'), coords = {x = coords.x, y = coords.y, z = coords.z}, description = text}
            TriggerClientEvent("qb-phone:client:addPoliceAlert", v.PlayerData.source, alertData)
            TriggerClientEvent('police:client:policeAlert', v.PlayerData.source, coords, text)
        end
...
RegisterNetEvent('police:server:SetPlayerOutVehicle', function(playerId)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"] then
        ...
        TriggerClientEvent("Cannot take out of the vehicle", 'error')
    end
end)
...
RegisterNetEvent('police:server:PutPlayerInVehicle', function(playerId)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"] then
        ...
        TriggerClientEvent("Cannot Place into the vehicle", 'error')
    end
end)
...
QBCore.Commands.Add('fine', 'Police Fine/Bill', {{name = 'id', help = 'Player ID#'}, {name = 'amount', help = 'Amount'}}, false, function(source, args)
            ...
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    if billed.Functions.RemoveMoney('bank', amount, "paid-fine") then
                        TriggerClientEvent('QBCore:Notify', source, "Fine Issued", 'success')
                        TriggerClientEvent('QBCore:Notify', billed.PlayerData.source, "Fine Issued")
                    ...
                    TriggerClientEvent('QBCore:Notify', source, "Amount must be higher", 'error')
                end
            else
                TriggerClientEvent('QBCore:Notify', source, "Cannot Fine Yourself", 'error')
            end
        else
            TriggerClientEvent('QBCore:Notify', source, "Player is not in city", 'error')
        end
    else
...
QBCore.Functions.CreateUseableItem('ziptie', function(source, args, message)
    TriggerClientEvent("NAT2K15:BACKZIP", source)
end)

QBCore.Functions.CreateUseableItem('ziptiecutter', function(source, args, message)
